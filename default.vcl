...
sub vcl_recv {
  ...
  } else if(req.url ~ "(people|constituencies|parties|parliaments|media|houses|contact_points|media|articles|groups)/\w{8}$") {
    set req.backend_hint = things;
  } else if(req.url ~ "(people|constituencies|parties|parliaments|media|houses|contact_points|media|articles|groups)/\w{8}(.xml|.json)$") {
    set req.backend_hint = things;
  } else if(req.url ~ "^/petition-a-hybrid-bill") {
    set req.backend_hint = things;
  } else if(req.url ~ "^/(people|constituencies|parties|parliaments|media|houses|contact_points|media|articles|groups)/lookup") {
    set req.backend_hint = things;
